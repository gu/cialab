...
var IIP = new Class({
   ...
   */
	...
	markers[0] = {"res":3,"tile":35,"x":2,"y":4,"obj":"dot"};
	markers[1] = {"res":3,"tile":37,"x":12,"y":4,"obj":"dot"};
	markers[2] = {"res":3,"tile":38,"x":2,"y":24,"obj":"dot"};
	markers[3] = {"res":3,"tile":32,"x":32,"y":4,"obj":"dot"};
	markers[4] = {"res":4,"tile":134,"x":32,"y":4,"obj":"dot"};
	markers[5] = {"res":4,"tile":133,"x":32,"y":4,"obj":"dot"};
	markers[6] = {"res":4,"tile":132,"x":32,"y":4,"obj":"dot"};
	markers[7] = {"res":4,"tile":119,"x":32,"y":4,"obj":"dot"};
	markers[8] = {"res":4,"tile":118,"x":32,"y":4,"obj":"dot"};
  ...
  loadGrid: function(){
    ...
    */
	...
	//alert(this.res + "  " + k); //test to make sure gets to this point

	
	for (x in this.markers)
	{
		if(this.res == this.markers[x].res && k == this.markers[x].tile)
		{
			var marker;
			marker = new Element('div', {
				styles: {
					...
					overflow: 'hidden',
					position: 'absolute',
					left: (i-startx)*this.tileSize[0] - xoffset + this.markers[x].x - 5,
					top: (j-starty)*this.tileSize[1] - yoffset + this.markers[x].y - 5,
					backgroundColor: 'rgb(94, 255, 0)',
					zIndex: 100
				}
			});
			marker.injectInside('target');
		}
	}
  ...
  createNavigationWindow: function() {
   ...
   var loadBarContainer = new Element('div', {
	...
	//$('addMarker').addEvent( 'click', this.zoomOut.bindWithEvent(this) );
